...
export class App extends React.Component<AppPropsType, AppStateType>{
	...
	componentDidMount() {
		axios.get<PostType[]>('http://jsonplaceholder.typicode.com/posts?_start=1&_limit=10')
			.then(({data}) => {
	...
	handleLoadPosts() {
		axios.get<Array<PostType>>(`http://jsonplaceholder.typicode.com/posts?_start=${this.state.lastPostStart}&_limit=10`)
		.then(({data}) => {
	...
	render() {
		return (
			<div>
				<div>
					<Counter value={this.state.counters.counter1} counters={this.state.counters} name='Counter 1' />
					<Counter value={this.state.counters.counter2} counters={this.state.counters} name='Counter 2'/>
				</div>
				<hr></hr>
